			...
			for( j = 0; j < n + 1; j++ )
			{
			    	...
			    	switch ( i % 2 ) 
				{
					...
					else 
					{
						...
						w = t1[j - 1] + ((a == 0) ? nuc_delta( yr[j - 1], x[i - 1] ) : prot_delta( yr[j - 1], x[i - 1] ));
					...
					else 
					{
						...
						w = t0[j - 1] + ((a == 0) ? nuc_delta( yr[j - 1], x[i - 1] ) : prot_delta( yr[j - 1], x[i - 1] ));
...
int decode_switches ( int argc, char * argv [], struct TSwitch * sw )
{
      ...
      while ( ( opt = getopt_long ( argc, argv, "a:i:o:O:E:h", long_options, &oi ) ) != - 1 )
      {
	switch ( opt )
	  {
	    case 'a':
...
void usage ( void )
{
	fprintf ( stdout, " Usage: Cyclic Needleman-Wunsch <options>\n" );
	fprintf ( stdout, " Standard (Mandatory):\n" );
	fprintf ( stdout, "  -a, --alphabet            <str>     ALphabet (DNA, RNA or PROT).\n" );
...
int main(int argc, char **argv)
{
	...
	char *           alphabet_letters;       // the letters in the alphabet
	...
	unsigned int     num_args;               // num args passed on command line
	unsigned int     a;                      // the matrix to use 0 = R/DNA, 1 = PROT
	...
	if ( num_args < 5 ) {
		...
		alphabet                = (char *) sw . alphabet;
		input_filename          = (char *) sw . input_filename;
		output_filename         = (char *) sw . output_filename;
	}
	...
	cyc_nw_ls ( a,  seq[0], m, seq[1], n, open_gap_penalty, extend_gap_penalty, &distance, &rotation );
	double end = gettime();
	...
	unsigned char * rot_str = ( unsigned char * ) calloc ( n + 1, sizeof ( unsigned char ) );
	create_rotation ( seq[1], rotation, rot_str );
	fprintf( out_fd, ">%s\n", seq_id[0] );
	fprintf( out_fd, "%s\n", seq[0] );
	fprintf( out_fd, ">%s\n", seq_id[1] );
	fprintf( out_fd, "%s\n", rot_str );
